...
bool LHHelixTrackFindingTask::Init()
{
  ...
  //FTDetector = (KBTpc *) (run -> GetDetectorSystem() -> GetTpc());
  fPadPlane = (KBPadPlane *) fTpc -> GetPadPlane();
  //fPadPlane2 = (KBPadPlane *) fTpc -> GetPadPlane();
...
int LHHelixTrackFindingTask::StepNewTrack()
{
  ...
  //kb_info << cnt << "  " << (Double_t)hit -> GetX() << "  " << (Double_t)hit -> GetY() << "  " << (Double_t)hit -> GetZ() << "  " << endl;
...
int LHHelixTrackFindingTask::StepInitTrackAddHit()
{
  ...
  if (quality > 0) {
    ...
    if (numHitsInTrack > fCutMaxNumHitsInitTrack) {
      ...
      //kb_info << iCand << "  " << (Double_t ) fCandHits -> GetHit(iCand) -> GetX() << "  " << (Double_t ) fCandHits -> GetHit(iCand) -> GetY() << "  " << (Double_t ) fCandHits -> GetHit(iCand) -> GetZ() << endl;
      }
